...
async function parseCompendium() {
  return new Promise<void>((resolve, reject) => {
    ...
    fs.createReadStream(
      join(__dirname, '../src/compendium/data/2024_Adult_Compendium_MET.csv'),
    )
      .pipe(csv())
      .on('data', (row) => {
        compendium.push(row);
      ...
      .on('end', () => {
        ...
        const stream = fs.createWriteStream(
          join(__dirname, '../src/compendium/data/compendium.csv'),
        );
...
async function main() {
  ...
  const compendiumPath = join(
    __dirname,
    '../src/compendium/data/compendium.csv',
  );

  if (fs.existsSync(compendiumPath)) {
    console.log('compendium.csv already exists.');
    rl.question(
      'Override default action (exit) and regenerate? (Y/n)\n',
      async (answer) => {
