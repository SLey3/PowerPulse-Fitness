...
export class FitexerciseService {
  ...
  async findDashboard(userId: number) {
    ...
    return tidy(
      results,
      groupBy(
        ['useCount', 'id', 'name', 'type'],
        [sliceMax(5, 'useCount'), arrange(desc('useCount'))],
      ),
    );
  }
  ...
  async findExerciseAnalyticsData(
    userId: number,
    all: boolean,
    opts: Prisma.ExercisesSelect | undefined,
  ) {
    const blacklist = {
      userId: true,
      notes: true,
  ...
  async createExercise(createDto: CreateDto, userId: number) {
    ...
    const met =
      (await this.compendium.findMET(createDto.type, createDto.name)) ||
      createDto.met ||
      '1';
  ...
  async updateUseCount(updateUseCountDto: UpdateUseCountDto, userId: number) {
    const incrementer_amount = updateUseCountDto.dir === 'pos' ? 1 : -1;

    const { id } = await this.prisma.exercises.findFirstOrThrow({
      where: {
        OR: [{ id: updateUseCountDto.id }, { name: updateUseCountDto.name }],
        userId: userId,
      },
    });
  ...
  async updateExercise(updateDto: UpdateDto, userId: number) {
    ...
    if (Object.keys(filteredDto).length === 0)
      throw new BadRequestException(
        'All Values remained the same. Update not Required',
      );
