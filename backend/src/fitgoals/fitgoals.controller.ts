...
import { CreateDto, UpdateDto, AnalyticsDto, uProgressDto } from './dto';
...
export class FitGoalsController {
  ...
  @Post('fetch/da')
  @HttpCode(200)
  findDataAnalyticsData(
    @Body() analyticsDto: AnalyticsDto,
    @GetUser('id') userId: number,
  ) {
    return this.fitgoalsService.findGoalAnalyticsData(
      userId,
      analyticsDto.all,
      analyticsDto.opts,
    );
  ...
  @Post('prog')
  updateGoalProgress(
    @Body() uProgDto: uProgressDto,
    @GetUser('id') userId: number,
  ) {
    return this.fitgoalsService.updateGoalProgress(uProgDto, userId);
  }
